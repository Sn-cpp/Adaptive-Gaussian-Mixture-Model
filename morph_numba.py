"""Parallel binary morphological operations on uint8 masks.

Every kernel is structured as a single prange(H) over rows so the
CUDA translation is mechanical:

    CPU  : @njit(parallel=True)  /  for y in prange(H)
    CUDA : @cuda.jit            /  y, x = cuda.grid(2)

All functions operate in-place on pre-allocated output arrays — no
