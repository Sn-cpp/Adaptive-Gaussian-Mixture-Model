"""Rother 2004 GrabCut GMM — full 3×3 covariance, 5 components.

This is a direct Numba translation of the GMM class in grabcut.cpp.
It is used for BOTH the foreground and background colour models that
feed t-link weights into the Push-Relabel graph cut.

Architecture (CUDA-ready)
--------------------------
All heavy passes are @njit kernels with a single prange(H) or
prange(N_pixels).  CUDA translation is mechanical:

    CPU  : @njit(parallel=True) / for y in prange(H)
    CUDA : @cuda.jit            / y, x = cuda.grid(2)

Arrays
------
model : (K, 13) float64   — component layout matching grabcut.cpp:
    [0]         coef  (mixing weight)
    [1:4]       mean  (B, G, R)
    [4:13]      cov   (3×3, row-major)
...
K = 5  # components (same as OpenCV GrabCut)
...
def _cov_inv_det(c, singular_fix):
    ...
    `c` is a (9,) float64 view.  Returns (det, inv_3x3).
...
def _eval_gmm(frame_bgr, model, inv_covs, cov_dets, out):
    ...
    frame_bgr : (H, W, 3) float64
    model     : (K, 13) float64
    inv_covs  : (K, 3, 3) float64
    cov_dets  : (K,) float64
...
def _accumulate_stats(frame_bgr, mask_gc, comp_idx, is_fg,
    ...
    sums  : (K, 3) float64   — colour sums per component
    prods : (K, 3, 3) float64 — outer-product sums per component
    counts: (K,) int64
...
# ── Python-level GMM class (fit + cache) ────────────────────────────────────
...
class GrabCutGMM:
    ...
    def __init__(self):
        self.model    = np.zeros((K, 13), dtype=np.float64)
        self.inv_covs = np.zeros((K, 3, 3), dtype=np.float64)
        self.cov_dets = np.zeros(K, dtype=np.float64)

        # Sufficient-statistics buffers (no alloc in hot path)
        self._sums   = np.zeros((K, 3),    dtype=np.float64)
        self._prods  = np.zeros((K, 3, 3), dtype=np.float64)
    ...
    def fit(self, frame_bgr_f64, mask_gc, is_fg):
        ...
        frame_bgr_f64 : (H, W, 3) float64
        mask_gc       : (H, W) uint8  — GC label map (0/1/2/3)
        is_fg         : bool
        """
    ...
    def neg_log_prob(self, frame_bgr_f64, out):
        """-log P(color | GMM) for every pixel → written into `out` (H,W) float32."""
        _eval_gmm(frame_bgr_f64, self.model, self.inv_covs, self.cov_dets, out)


def warmup_fgd_gmm(H=4, W=4):
    ...
    model    = np.zeros((K, 13),    dtype=np.float64)
    inv_covs = np.zeros((K, 3, 3),  dtype=np.float64)
    cov_dets = np.zeros(K,          dtype=np.float64)
    ...
    prods  = np.zeros((K, 3, 3), dtype=np.float64)
